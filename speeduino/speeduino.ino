    ...
    if (BIT_CHECK(currentStatus.LOOP_TIMER, BIT_TIMER_4HZ))
    {
      ...
      if(BIT_CHECK(statusSensors, BIT_SENSORS_AUX_ENBL))
      {
        ...
        for (byte AuxinChan = 0; AuxinChan <16 ; AuxinChan++)
        {
          ...
          // External CAN / secondary-serial-relayed aux input channels were removed from
          // this fork along with CAN/secondary serial support. With no external source
          // available, TunerStudio stores the Local Aux Input Channel selection in bits
          // [0:1] of caninput_sel (see caninput_sel0a etc. in the .ini): 2 = analog local
          // pin, 3 = digital local pin.
          if ((configPage9.caninput_sel[currentStatus.current_caninchannel]&3) == 2)
          ...
          else if ((configPage9.caninput_sel[currentStatus.current_caninchannel]&3) == 3)
